    ...
    <div class="row mb-4">
        <div class="col-lg-12">
            <h2 class="fw-bold text-dark">
                <i class="fas fa-calendar-edit me-2 text-info"></i>Edit Jadwal Latihan
            </h2>
        </div>
    </div>
        ...
        <div class="col-lg-8">
            <div class="card border-0 shadow-sm">
                <div class="card-body p-4">
                    <form action="{{ route('admin.fitness.schedules.update', $schedule) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('sport_id') is-invalid @enderror" 
                                ...
                                @foreach($sports as $sport)
                                    <option value="{{ $sport->id }}" {{ old('sport_id', $schedule->sport_id) == $sport->id ? 'selected' : '' }}>
                                        {{ $sport->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="date" class="form-control @error('schedule_date') is-invalid @enderror" 
                                   id="schedule_date" name="schedule_date" value="{{ old('schedule_date', $schedule->schedule_date?->format('Y-m-d')) }}" required>
                            @error('schedule_date')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control @error('max_participants') is-invalid @enderror" 
                                   id="max_participants" name="max_participants" value="{{ old('max_participants', $schedule->max_participants) }}" 
                                   min="1" required>
            ...
            <div class="card border-0 shadow-sm bg-light">
                <div class="card-body">
                    ...
                    <div class="alert alert-info" role="alert">
                        <small>
                            <p><strong>Dibuat:</strong> {{ $schedule->created_at->format('d-m-Y H:i') }}</p>
                            <p><strong>Terakhir Diubah:</strong> {{ $schedule->updated_at->format('d-m-Y H:i') }}</p>
                        </small>
                    </div>
                </div>
            </div>
